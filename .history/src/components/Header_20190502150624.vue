<template>
  <header class="header">
    ...
    <ul>
      <li v-for=" (item,index) in headers" :key="index" class="animated" :class="{'bounce':index === clickNo,'pulse':index === hoverNo}" @mouseover="hoverNo = index" @click="toggle(index,item.target)">
        {{item.label}}
      </li>
    </ul>
  </header>
  ...
  methods: {
    ...
    onScroll() {
      ...
      this.nodeDistance.forEach(({ offsetTop }, index) => {
        if (index > 1) return;
        const nextOffsetTop = this.nodeDistance[index + 2];
        if (nextOffsetTop) {
          if (
            offsetTop <= scrolled &&
            scrolled < (nextOffsetTop.offsetTop || 10000)
          ) {
            this.clickNo = index;
          }
